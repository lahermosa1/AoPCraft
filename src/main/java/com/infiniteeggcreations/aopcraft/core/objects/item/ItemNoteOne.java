...
import com.infiniteeggcreations.aopcraft.core.client.gui.GuiNoteOne;
 ...
 *
 * This item is used to display a note on the player screen (HUD).
 */
public class ItemNoteOne extends Item implements IHasModel
{
    // ========== Constructor ==========
    public ItemNoteOne(String name)
    {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn)
    {
        if(!worldIn.isRemote)
        {
            playerIn.sendMessage(new TextComponentString("Trust the Mages..."));
        }
        else
            {
                // Right clicking the note, the message will appear on the HUD:
                Minecraft.getMinecraft().displayGuiScreen(new GuiNoteOne());
            }
